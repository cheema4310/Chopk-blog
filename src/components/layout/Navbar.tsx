"use plain"; 
import Link from "next/link";
import { Button } from "../ui/Button";
import { cn } from "@/lib/utils";
import Image from "next/image";

export function Navbar() {
  return (
    <nav className="fixed top-4 left-1/2 -translate-x-1/2 w-[95%] max-w-5xl z-50">
      <div className={cn(
        "glass rounded-full px-6 py-3 flex items-center justify-between transition-all duration-300",
        "shadow-[0_4px_30px_rgba(0,0,0,0.1)] border border-white/10 hover:border-white/20 hover:shadow-[0_0_20px_rgba(255,255,255,0.05)]"
      )}>
        {/* Logo */}
        <Link href="/" className="relative group p-1">
          <div className="absolute inset-0 bg-accent/20 blur-[20px] rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-500 scale-150" />
          <Image 
            src="https://res.cloudinary.com/dcr0te1k4/image/upload/v1771111380/chopk-logo-kegel-app-upload_opqcda.png"
            alt="ChopK"
            width={120}
            height={42}
            className="h-[42px] w-auto relative z-10 transition-transform duration-300 group-hover:scale-105 drop-shadow-[0_0_10px_rgba(238,112,35,0.2)]"
            priority
          />
        </Link>

        {/* Desktop Links */}
        <div className="hidden md:flex items-center gap-8">
          <Link href="/" className="text-sm font-medium text-sub-text hover:text-white transition-colors tracking-wide hover:drop-shadow-[0_0_8px_rgba(255,255,255,0.5)]">
            Home
          </Link>
          <Link href="/blog" className="text-sm font-medium text-sub-text hover:text-white transition-colors tracking-wide hover:drop-shadow-[0_0_8px_rgba(255,255,255,0.5)]">
            Blog
          </Link>
        </div>

        {/* CTA */}
        <div className="flex items-center gap-4">
          <Link href="https://play.google.com/store/apps/details?id=com.chopk.app" target="_blank">
             <Button variant="primary" size="sm" className="hidden sm:flex neon-glow">
               Try ChopK
             </Button>
          </Link>
        </div>
      </div>
    </nav>
  );
}
